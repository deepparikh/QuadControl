...
%Set up Plot
plotGraph = plot(time,data,'-mo','LineWidth',1,'MarkerEdgeColor','k','MarkerFaceColor',[0.49 1 0.63],'MarkerSize',2);
...
while ishandle(plotGraph) %Loop when Plot is Active
    ...
    if(~isempty(dat) && isfloat(dat)) %Make sure Data Type is Correct        
        ...
        axis([time(count)-scrollWidth time(count)+(scrollWidth/2) min max]);
        else
...
fclose(s);
clear count delay max min plotGraph plotGrid plotTitle s ...
